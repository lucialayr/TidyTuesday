...
(p = ggplot() + coord_polar() + theme_bw() +
  ...
  scale_y_continuous(name = '', expand = c(0,0), breaks = c(10000, 20000), limits = c(0,30000)) +
  scale_x_continuous(expand = c(0,0),  limits = c(30, 330), labels = c('Spring', 'Fall'), 
                     breaks = c(yday('2010-03-30'), yday('2010-09-30'))) +
  scico::scale_color_scico(palette = 'devon', name = '        Year', breaks = c(1994, 2024), end = .9) +
  theme(axis.line = element_blank(),
        ...
        panel.grid.major.y = element_line(linetype = 'dotted', linewidth = .25),
...
ggdraw(p) + 
  draw_image(logo_file, x = 0.5, y = 0.535, hjust = 0.5, vjust = 0.5, width = 0.5, scale = 1.3)
